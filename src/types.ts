...
export interface RaycastPreferences {
  ...
  export interface TableauView extends TableauItemBase {
    itemType: 'View';
    workbook?: { // Information about the workbook to which the view belongs
      id: string;
      name?: string;
      contentUrl?: string;
    };
  ...
  export interface ApiErrorDetail { // This type is not used directly in the code, but is useful for understanding structure
    code: string;
    summary: string;
    detail: string;
  }
  
  export interface ApiError { // Used in API responses
