...
type EffectivePreferences = Required<Pick<StoredPrefs, 'tableauServerUrl' | 'tableauApiVersion' | 'personalAccessTokenName' | 'personalAccessTokenSecret'>> & {
...
export async function getTableauAuthToken(): Promise<{ token: string; siteLuid: string }> {
  try {
    ...
    try {
      const response = await fetch(`${effectivePrefs.tableauServerUrl}/api/${effectivePrefs.tableauApiVersion}/auth/signin`, {
        method: "POST",
      ...
      const data = (await response.json()) as { credentials: { token: string; site: { id: string; contentUrl: string } } };
...
export async function searchTableauItems(query?: string, limit?: number): Promise<(TableauView | TableauWorkbook)[]> {
  ...
  if (limit === 1) {
    try {
      ...
      const response = await tableauApiRequest<{ workbooks: { workbook: TableauWorkbook[] } }>('/workbooks?pageSize=1&fields=id,name');
  ...
  const workbookFieldsList = [
    "contentUrl", "description", "size", "defaultViewId", "createdAt",
    "hasExtracts", "updatedAt", "name", "sheetCount", "showTabs",
    "shareDescription", "id", "lastPublishedAt", "tags", "primaryContentUrl",
    "project.id,project.name", "owner.id,owner.name"  // Request explicit subfields
  ];
  ...
  while(moreWorkbooksAvailable) {
    try {
      const workbookResponse = await tableauApiRequest<{ workbooks: { workbook: TableauWorkbook[] }, pagination: { totalAvailable: string } }>(
      ...
      if (workbooks.length >= totalAvailable || (workbookResponse.workbooks.workbook && workbookResponse.workbooks.workbook.length < pageSize) || !workbookResponse.workbooks.workbook) {
  ...
  const viewFieldsList = [
    "id", "name", "contentUrl", "createdAt", "updatedAt", 
    "owner.id,owner.name", "project.id,project.name",
    "tags", "viewUrlName", "workbook.id,workbook.name,workbook.contentUrl", 
    "sheetType"
  ];
  ...
  while(moreViewsAvailable) {
    try {
      const viewResponse = await tableauApiRequest<{ views: { view: TableauView[] }, pagination: { totalAvailable: string } }>(
      ...
      if (views.length >= totalAvailable || (viewResponse.views.view && viewResponse.views.view.length < pageSize) || !viewResponse.views.view) {
